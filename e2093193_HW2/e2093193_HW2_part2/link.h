...
void LinkedList<T>::InsertAfter(const T& item)
{
    ...
    else
    {
        ...
        if (currPtr == NULL)
            currPtr = prevPtr;
        currPtr->InsertAfter(p); // insert the node after currPtr
        if (currPtr == rear)
        {
            rear = p;           //when it is the last node make the new pointer p point to rear and make position equal to sizes
            position = size;
        }
        else
            position++;
